---
title: 数据结构：树的概念与储存
date: 2023-07-27 19:34:23 +08:00
tags:
    - 数据结构
    - 基础算法
    - 树
...
树的结点一般画为圆圈，树若画出来也就是几个结点（圆）和几条边（线）组成的图，比较形象化，实际的树的储存方式是通过[链表](/posts/lianbiao)中的数据域将一个个结点（元素）连接起来，比较麻烦，后面再说。  
...
一般来说，这些方法用不太到：要么炸时间，要么炸空间，要么难实现。所以就按这种奇特的分类方法分类了，反正感觉用不到。这句话也兼下面。
...
**邻接矩阵表示法**：见[这里](/posts/graph-search/#邻接矩阵一般不用)。  
**邻接表表示法**：也看上面。
