...
date: 2023-08-12 10:00:41 +08:00
math: true
---
...
其实，任何进制之间的转换都可以将那个进转换为十进制后再转换为目标进制。十进制也可以转换为任何进制。至于怎么转换，其实和[二进制和十进制之间的转换](#二进制和十进制的转换)差不多。  
...
:::note{.info} P.S. 二进制、八进制、十进制、十六进制在 C++ 中的表示方法（前缀）
...
:::note{.info} P.S. 位运算时赋值

同 `+=` `-=` 等符号一样，位运算也可以在符号后面加上 `=`，`>>=` `^=` `&=` `<<=` `|=` 等运算符都是可以的。

:::

:::note{.info} P.S. 位运算的一些使用技巧
...
在原码中，除符号位外，剩下的二进制都是按照[二进制转十进制](#二进制---十进制)一样。若符号位是 1 那就将转换的十进制加个负号。例如：
